...
async function fetchV2ChartData() {
  let data: {
    date: number
    dailyVolumeUSD: string
    totalLiquidityUSD: string
  ...
  try {
    while (!allFound) {
      const { data: chartResData, error, loading } = await v2Client.query<ChartResultsV2>({
        query: GLOBAL_CHART_V2,
        variables: {
          startTime: startTimestamp,
          skip,
        },
 ...
 * Fetch historic chart data
 */
export function useFetchV2GlobalChartData(): {
  error: boolean
  data: ChartDayData[] | undefined
} {
  ...
  useEffect(() => {
    async function fetch() {
      const { data, error } = await fetchV2ChartData()
      if (data && !error) {
        setData(data)
      } else if (error) {
        setError(true)
